...
class KMVGAE(nn.Module):
    def __init__(self, **kwargs):
        ...
        # self.gcn_logstddev = nn.ModuleList([GraphConv(hid1_dim, self.hid2_dim, bias = False) for i in range(len(self.view_adj))])
        self.gcn_logstddev = nn.ModuleList([FLPASS(hid1_dim, self.hid2_dim, 0.2, 0.8) for i in range(len(self.view_adj))])
    ...
    def _train(self):
        ...
        optimizer = torch.optim.Adam(self.parameters(),lr=0.0005, weight_decay=0.10)
        ...
        for epoch in epoch_bar:
            ...
            for i in range(len(self.view_adj)):
                ...
                loss_comp = -0.5 * torch.log(torch.det(e - 0.0006 * tmp_z @ tmp_z.T))  # .sum()
                loss = loss + loss_comp
